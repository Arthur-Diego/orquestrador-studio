...
> **Cópia instalada no projeto.** O original fica em
> `skills_proprias/visual-dna-moodboard/` (organização do dono do repo).
> Único delta desta cópia: `Bash` e `Write` no `allowed-tools` — a skill manda rodar
> `validate_visual_dna.py` (precisa de Bash) e o `mood_orquestrador` precisa que ela grave
> o `dna.json` que o `mood_board_builder` consome. O resto é idêntico ao original.
