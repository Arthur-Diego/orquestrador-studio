...
Versão: 1.5 (Onda A + sincronização chat → telas + feedback ao vivo do turno + navegação
automática + `ask` com ações + entrada por voz)
Data: 2026-09-06
Task-Id: ADH-OS-20260905-04 (v1.0) · ADH-OS-20260906-05 (v1.1) · ADH-OS-20260906-04 (v1.2) ·
ADH-OS-20260906-10 (v1.3) · ADH-OS-20260906-13 (v1.4) · ADH-OS-20260906-11 (v1.5)
Responsável: Arthur Diego (modo autônomo /dd-parallel, aprovação total)

> **v1.5 (Wave 11 · F09, card #89, ADR-043)** — o composer deixa de ser só um textarea: o usuário
...
> (v1.5)" abaixo. FDD: `docs/domains/chat/features/chat-audio-fdd.md`.
...
Acrescentados na **v1.5** (Wave 11 · F09):
...
| `POST /api/chats/{id}/transcribe` | REST (**`multipart/form-data`** — a primeira do domínio) | v1.5: áudio falado → `{text, provider, duration_s}`. Campos `file` (singular) e `duration_s`; tetos de 10 MB e 120 s. `404` aba inexistente · `409` só o fake disponível · `413` grande demais · `422` formato/assinatura/duração · `502` `ProviderError`; `detail` é sempre string. Nada é enviado nem gravado aqui (ADR-043) |
...
hatch da sonda de `--include-partial-messages`, v1.2). A entrada por voz (v1.5) depende de
...
- Da voz (v1.5): transcrição no navegador (Web Speech API — **rejeitada** na ADR-024, reabrir exige
