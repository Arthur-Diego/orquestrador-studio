...
| `service.py` | criar/listar projetos (layout do curso), `suggest_terms`, jobs de busca e login, `select` → `brainstorming/` + `README.md`, validação de `pid` | `pinterest.py`, `config` |
...
- `POST /refs/select {ids,notes}` → copia escolhidas para `refs/brainstorming/`, remove desmarcadas, escreve `README.md`.
