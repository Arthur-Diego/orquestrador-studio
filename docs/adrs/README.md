...
_ADR-025 (2026-08-31, Wave 9 sub-wave 2): a etapa 4 passou a OFERECER roteiro completo por LLM — papel `script` no prompter (Claude CLI local, grátis), N cenas com texto pt-BR e prompt de imagem em inglês no formato briefing de diretor de fotografia, aplicando o rig do preset de realismo da ADR-016 estendida pela feature irmã. É `[extensão]` que **contraria explicitamente** a docstring da própria etapa ("nada de roteiro por LLM"), por isso existe: a sugestão vive isolada em `storyboard/script.json`, o servidor nunca escreve `scenes.json` (ADR-018/022 preservados), a aplicação é assimétrica (preencher cena vazia sem diálogo; sobrescrever texto do usuário só com confirmação que diz quantos serão perdidos) e não há fallback determinístico (sem Claude CLI é 409 — o método manual da aula 010 continua sendo o caminho padrão). Registra também o **default de preset ATIVO** de `storyboard.script` (`documentary-street`), exceção consciente à regra opt-in que a `prompter-presets-realismo` fixou para `mood`/`base`/`motion`. Estende ADR-004 e relaciona ADR-006, ADR-008, ADR-010, ADR-015, ADR-016 e ADR-018/022._

_ADR-028 (2026-08-31, ADH-OS-20260831-B-roteiro-cena): a Frente B do storyboard estendeu o roteiro por LLM da ADR-025 em duas frentes aditivas. (1) O roteiro passa a INFERIR quantas fotos cada cena pede (`shots`, faixa 3–6, julgado pelo modelo a partir da descrição) e a escrever prompts coesos entre si por cena (`shot_prompts`, mesma cena = mesmo local/produto/paleta/luz, variando só momento/enquadramento); o servidor só normaliza a faixa e nunca inventa acima do que veio (`_normalize_shots`), `image_prompt` vira a primeira foto (compat), o rig do preset é cobrado em TODAS as fotos e `scenes.json` segue intocado — o encaixe das fotos no roteiro continua MANUAL. (2) O painel "Roteiro por Claude" passou a preceder "A história em cenas" no stepper (roteiro=02, história=03, ângulos=04, cena=05), porque o roteiro é quem origina as cenas; a renumeração é só ordem física de `<section>` + strings "painel NN", sem tocar o opt-in `PUT /scenes`. Estende ADR-004 e ADR-025; relaciona ADR-006, ADR-008, ADR-010, ADR-015, ADR-016 e ADR-018._
