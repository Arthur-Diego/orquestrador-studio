...
def test_etapa_aparece_pronta_no_catalogo(client):
    ...
    # Wave 10 · E4 (ADR-032): a tela migrou para React (`studio/etapas/publish/ui/index.tsx`); os
    # `view.{html,js}` saíram. DOM/comportamento e textos de aula → substituto Vitest
    # `studio/etapas/publish/ui/index.test.tsx` (C-PUBLISH-*).
...
# Wave 10 · E4 (ADR-032): `test_view_html_segue_a_aula_sem_copy_automatica` e
# `test_view_segue_o_catalogo_do_redesign` liam o fonte de `publish/view.{html,js}` (substring sobre
# a tela vanilla). A tela virou React (`publish/ui/index.tsx`); o contrato de DOM/comportamento, o
# conjunto exato de campos do formulário e os textos de aula (ADR-004) passam a ser verificados pelo
# substituto Vitest `studio/etapas/publish/ui/index.test.tsx` (C-PUBLISH-*) e pelo diff de
# `textContent` do baseline da E0.
